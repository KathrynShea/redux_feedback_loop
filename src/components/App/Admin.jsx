...
console.log("allfeedback is", allFeedback);
    ...
    <table class="table table-hover">
        ...
        <tbody>
            {allFeedback.length !== 0 && allFeedback.map((item, index) => {
            return <tr key={index} class="table-dark"><td>{item.feeling}</td><td>{item.understanding}</td><td>{item.support}</td><td>{item.comments}</td></tr>})}
